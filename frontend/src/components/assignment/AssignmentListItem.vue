<template>
  <div class="bg-white rounded-lg shadow-sm p-4 border border-gray-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-gray-50 transition-colors">
    <div class="flex-grow self-start md:self-center">
      <p v-if="courseName" class="text-xs text-gray-600 mb-1">{{ courseName }}</p>
    ...
    <div class="flex items-center gap-2 md:gap-4 flex-shrink-0 w-full md:w-auto justify-end">
      ...
      <button
        ...
        class="inline-flex items-center justify-center gap-2 bg-green-700 text-white text-sm font-medium h-9 w-24 rounded-md hover:bg-green-600 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
      >
