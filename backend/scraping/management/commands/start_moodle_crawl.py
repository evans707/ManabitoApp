...
from scraping.crawlers.spiders.moodle_spider import MoodleSpider
...
MOODLE_LOGIN_URL = os.getenv('MOODLE_LOGIN_URL')

# 環境変数が設定されているか確認
if not all([USERNAME, PASSWORD, MOODLE_LOGIN_URL]):
    logging.error("エラー: .envファイルに USERNAME, PASSWORD, MOODLE_LOGIN_URL のいずれか、または複数が設定されていません。")
    exit() # スクリプトを終了

User = get_user_model()

class Command(BaseCommand):
    help = '環境変数で指定されたユーザーのMoodle課題をスクレイピングします'
